...
RSpec.describe 'Memberships API', type: :request do
  ...
  before do
    stub_request(:get, "#{ENV['EXTERNAL_SERVICE_BASE_URL']}/users/#{user_id}")
      .to_return(status: 200, body: { id: user_id, name: 'John Doe' }.to_json, headers: { 'Content-Type' => 'application/json' })
  end

  context 'when the assignment is successful' do
    before do
      stub_request(:get, "#{ENV['EXTERNAL_SERVICE_BASE_URL']}/teams/#{team_id}")
        .to_return(status: 200, body: { id: team_id, name: 'Team Alpha', teamMemberIds: [user_id] }.to_json, headers: { 'Content-Type' => 'application/json' })
    end
  ...
  context 'when the user does not belong to the team' do
    before do
      ...
      stub_request(:get, "#{ENV['EXTERNAL_SERVICE_BASE_URL']}/teams/#{team_id}")
        .to_return(status: 200, body: { id: team_id, name: 'Team Alpha', teamMemberIds: ['some-other-user-id'] }.to_json, headers: { 'Content-Type' => 'application/json' })
    end
